...
class DecisionTree(object):
    ...
    def fit(self, train_feature, train_label, val_feature, val_label):
        ...
        val_feature = val_feature.tolist()
    ...
    def predict(self, feature):
        ...
        # 单个样本分类（参考calc_acc_val中的classify函数）
        def classify(tree, sample):
            # 如果tree不是字典，说明是叶子节点，直接返回标签
            if not isinstance(tree, dict):
                return tree
            ...
            t_index, t_value = list(tree.items())[0]
            # 获取样本在该特征上的值
            f_value = sample[t_index]

            # 如果t_value是字典，继续递归
            if isinstance(t_value, dict):
                classLabel = classify(tree[t_index][f_value], sample)
                return classLabel
            else:
                # t_value本身就是标签值
                return t_value
        ...
        for sample in feature:
            result.append(classify(self.tree, sample))
