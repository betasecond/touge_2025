...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        def predictOne(tree, sample):
            # 如果tree是叶子节点（不是字典），直接返回标签
            if not isinstance(tree, dict):
                return tree

            # 获取当前节点的特征索引（树的根节点key）
            ...
            if feature_value in sub_tree:
                # 递归预测
                return predictOne(sub_tree[feature_value], sample)
            else:
                # 如果特征值不在训练集中出现过，返回None或默认值
                # 这里可以返回出现最多的标签，但简单起见返回None
                return None

        # 对所有测试样本进行预测
        result = []
        for sample in feature:
            result.append(predictOne(self.tree, sample))
